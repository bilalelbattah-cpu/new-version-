...
# Function to load categories and sub-categories from Feuil1
@st.cache_data
def load_categories():
    try:
        df = pd.read_excel(EXCEL_FILE, sheet_name='Feuil1', engine='openpyxl')
        categories = {}
        current_cat = None
        for _, row in df.iterrows():
            cat = row.get('Catégorie', None)
            sub = row.get('sous catégorie', None)
            if pd.notna(cat):
                current_cat = cat
        ...
        st.error(f"Erreur lors du chargement des catégories depuis Feuil1 : {e}")
        # Fallback to hardcoded categories if Excel fails
        return {
...
if page == "Ajouter un Achat":
    ...
    with st.form(key='add_purchase'):
        ...
        cat = st.selectbox("Catégorie", options=list(categories.keys()), key='category_select')
        sub_cat_options = categories.get(cat, [])
        sub = st.selectbox("Sous-catégorie", options=sub_cat_options, key='subcategory_select')
...
# Page 3: Budget
    ...
    with st.form(key='set_budgets'):
        for cat in categories:
            st.session_state.budgets[cat] = st.number_input(f"Budget pour {cat} (MAD)", value=st.session_state.budgets[cat], min_value=0.0, step=10.0)
    ...
    # Chart for budget status
    if not budget_df.empty:
        chart_data = budget_df.set_index('Catégorie')[['Budget', 'Dépensé']]
        st.subheader("Visualisation du Budget")
...
# Page 4: Synthèses Mensuelles
    ...
    if not df_data.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name="rapport_mensuel.csv",
            mime="text/csv"
        )
    else:
        st.info("Aucune donnée pour les synthèses.")